        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-6">
                    <img class="img-responsive center-all-contens" src="assets/img/bien.png" style="opacity: .4">
                </div> 
                <div class="col-xs-12 col-sm-6">
                    <div id="form-compra">
                        <form action="process/confirmcompra.php" method="post" role="form" class="FormCatElec" data-form="save">
                            <?php
                                if(!$_SESSION['nombreUser']=="" &&!$_SESSION['claveUser']==""){
                                          //<input type="text" name="clien-pass" value="'.$CodigoProducto.'">
                                    echo '
...
$query="SELECT Nombre  FROM cliente cli 
  INNER JOIN venta ven ON cli.NIT=ven.NIT
  INNER JOIN detalle det ON det.NumPedido=ven.NumPedido
  INNER JOIN producto prod on prod.CodigoProd=det.CodigoProd WHERE CodigoProd='".$CodigoProducto."'";
  ...
  while($row=$resultado->fetch_assoc())
  {
    ...
    echo '
    <input type="text" name="vendedor" value="'.$row['Nombre'].'">
    ';
      //echo $row['Nombre'];
    //echo "<script>alert($row['Nombre']);</script>";
    //echo 'alert(\'' + $row['Nombre'] + '\');';
    //echo("<script>console.log('PHP: ".$row['Nombre']."');</script>");
    //print_r($row['Nombre']);
  }
  ...
  */
                                    ...
                                    echo '
                                        ...
                                        <p>
                                            ...
                                            y contraseña con la cual te registraste en <span class="tittles-pages-logo">TiendaFcc</span>.
                                        </p>
                                        <br>
