...
if($num>0){
  ...
  $StatusV="Pendiente";
  //$variable = $_POST['admin-name'];
  
  /*Insertando datos en tabla venta*/


  //$Nombreinfo=ejecutarSQL::consultar("select * from producto where Nombre = '".$nombreP."'");
/*echo "<script>alert('<?php echo $nombreP; ?>');</script>"; */
  
  
  consultasSQL::InsertSQL("venta", "Fecha, NIT, Descuento, TotalPagar, Estado, Vendedor", "'".date('d-m-Y')."','".$nitC."','0','".$_SESSION['sumaTotal']."','".$StatusV."','".$nombreP."'");
